...
import { CheckCircle2, Clock, Briefcase, Activity, MapPin, Inbox, Receipt } from 'lucide-react';

const sum = (jobs: Job[], pick: (j: Job) => number) => jobs.reduce((t, j) => t + pick(j), 0);

type Filter = 'all' | 'PAID' | 'COMPLETED' | 'active';
...
export default function BusinessStatsPage() {
  ...
  const m = useMemo(() => {
    ...
    const active = inMonth.filter((j) => ['OPEN', 'ACCEPTED', 'IN_PROGRESS'].includes(j.status));
    return {
      inMonth, paid, completed, done, active,
      spend: sum(done, (j) => j.grossPriceCents),
      paidSpend: sum(paid, (j) => j.grossPriceCents),
      pendingSpend: sum(completed, (j) => j.grossPriceCents),
      avgCost: done.length ? Math.round(sum(done, (j) => j.grossPriceCents) / done.length) : 0,
    };
  }, [jobs, month]);
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">סקירת הוצאות ופעילות לפי חודש</p>
        </div>
      ...
      {isLoading ? (
        ...
        <>
          <StatHero
            icon={Receipt}
            label={<>הוצאות · <span className="capitalize">{format(month, 'MMMM yyyy', { locale: he })}</span></>}
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <StatsCard title="עבודות פעילות" value={m.active.length} icon={Activity} tone="warning" />
            <StatsCard title="עבודות החודש" value={m.inMonth.length} icon={Briefcase} tone="info" />
          </div>
                  ...
                  <div key={job.id} className="flex items-center gap-4 p-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 flex items-center gap-1.5 truncate text-xs text-muted-foreground">
                        ...
                        {job.driver && <span className="text-muted-foreground/70">· {job.driver.name}</span>}
                      </p>
                    </div>
                    <div className="flex shrink-0 flex-col items-end gap-1">
                      <p className="text-sm font-bold">{formatPrice(job.grossPriceCents)}</p>
